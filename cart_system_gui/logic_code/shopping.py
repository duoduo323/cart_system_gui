...
class ShoppingWindow(QMainWindow):
    def __init__(self):
        # 调用父类的初始化方法
        super(ShoppingWindow, self).__init__()
    ...
    def add_cart(self):
        ...
        # logger_file('info',"普通用户正在添加商品到购物车...")
        try:
            if self.item:
    ...
    def delete_cart(self):
        try:
            # logger_file('info', "正在读取购物车数据...")
            cart = file_read('cart.txt')
            self.cart_select_item = self.cart_table.selectedItems()

            # 获取所有要删除的行号（去重）
            rows_to_delete = set()
            for item in self.cart_select_item:
            ...
            for row in rows_to_delete:
                ...
                quantify_item = self.cart_table.item(row, 2)  # 第0列是商品名称列
                if name_item:
    ...
    def check_cart(self):
        ...
        logger_file('warning',f"该普通用户结算成功！成功消费￥{total:.2f}元")
