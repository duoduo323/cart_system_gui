...
class CargomanagementWindow(QMainWindow):
    def __init__(self):
        # 调用父类的初始化方法
        super(CargomanagementWindow, self).__init__()
    ...
    def alladd(self):
        try:
            logger_file('info', "管理员进行了一键添加，将采购清单中不在正在销售阶段的商品全部添加到正在销售阶段...")
            lst = []
            ...
            for i in lst:
                ...
                logger_file('warning', f"管理员一键添加操作成功添加商品：{i.get('name')}！")
            self.startsale()
        except Exception as e:
            logger_file('error', f"一键添加商品失败！错误为{e}")

    def addcargo(self):
        self.close()
        self.new_cargo.show()
        logger_file('info', "管理员开始往采购清单中添加新商品...")

    def delcargo(self):
        try:
            logger_file('info', "正在读取采购清单中的数据...")
            ...
            for i in items_to_delete:
                logger_file('warning', f"成功从采购清单中删除商品{i}并更新了数据库!")
        except Exception as e:
            logger_file('error', f"从采购清单中删除商品失败！错误为{e}")
    ...
    def delete(self):
        ...
        if self.item:
            ...
            self.startsale()
            logger_file('warning', f"管理员成功将正在销售的商品{name}下架并更新了数据库！")
    ...
    def push(self):
        ...
        if self.items:
            ...
            self.startsale()
            logger_file('warning', f"管理员成功将正在采购清单中的商品{name}上架并更新了数据库！")

    def fill_purchase(self):
        # logger_file('info', "开始读取采购清单中的商品...")
        ...
        # logger_file('info', f"成功读取{len(shopping_data)}件商品")
        # for col in range(2):
        #     if col == 0:
...
if __name__ == "__main__":
    ...
    cargomanage = CargomanagementWindow()
    cargomanage.show()
    sys.exit(app.exec_())
